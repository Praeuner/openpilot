...
export const useRoutesStore = create<RoutesState>((set) => ({
  ...
  fetchRoutes: async (page = 1) => {
    ...
    try {
      ...
      if (Array.isArray(response)) {
        routes = response
      } else if (response && typeof response === 'object' && 'routes' in response) {
  ...
  fetchRouteDetails: async (routeId: string) => {
    ...
    try {
      ...
      set({
        error: error instanceof Error ? error.message : 'Failed to fetch route details',
        loading: false,
      })
      return null
    }
  ...
  deleteRoute: async (routeId: string) => {
    try {
      ...
      set((state) => ({
        routes: state.routes.filter((r) => r.baseName !== routeId && r.id !== routeId),
        selectedRoute: (state.selectedRoute?.baseName === routeId || state.selectedRoute?.id === routeId) ? null : state.selectedRoute,
      }))
    } catch (error) {
      set({
