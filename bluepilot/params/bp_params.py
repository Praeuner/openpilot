  ...
  try:
    ...
    try:
      if param_type == "bool":
        ...
        debug(f" - Retrieved string parameter {param_name}: {value}")
        return value
    except ValueError:
...
def apply_custom_params(obj: Any, prop_key: str, component_type: str) -> None:
  ...
  for param in params_json.get("params", []):
    if prop_key in param and component_type == prop_key.replace("Prop", "").lower():
      ...
      try:
        ...
        if param_type == "int" and value is not None:
          try:
            ...
            if min_value is not None and value < min_value:
              value = min_value
              log_debug(f"Clamped {param_name} to min value: {min_value}")
            if max_value is not None and value > max_value:
              value = max_value
              log_debug(f"Clamped {param_name} to max value: {max_value}")
          except ValueError:
          ...
          try:
            ...
            log_debug(f"Invalid float value for {param_name}, using default: {default_value}")
            value = default_value
        ...
        if hasattr(obj, attr_name):
          ...
          log_debug(f"Initializing parameter '{param_name}' to '{value}' in {component_type}")
  ...
  if params_changed > 0:
    ...
    for change in changed_params:
      if change["old_value"] != "UNSET":
        log_debug(f"  • {change['param_name']} ({change['attr_name']}): {change['old_value']} → {change['new_value']}")
      else:
        log_debug(f"  • {change['param_name']} ({change['attr_name']}): initialized to {change['new_value']}")
  else:
