...
from opendbc.car.ford.values import CarControllerParams, FORD_VEHICLE_TUNINGS
...
SETTINGS_PARAMS = [
  ...
  ["FordLatTuningLaneChangeFactorHighUI", "lane_change_factor_high", 0.90, float],
  ["FordLatTuningPCBlendRatioUI", "pc_blend_ratio_UI", 0.10, float],
...
def initialize_param_defaults(self_obj):
  for param_name, self_key_name, default_value, param_type in SETTINGS_PARAMS:
    ...
    if hasattr(self_obj, self_key_name):
      ...
      reload_param(self_obj, param_name, self_key_name, param_type)
...
def reload_param(self_obj, param_name, self_key_name, param_type):
    initial_val = getattr(self_obj, self_key_name,)
    value = get_param_value(param_name, param_type)
    logDebug(f"Param '{param_name}' changed from '{initial_val}' to '{value}'.") if initial_val != value else None
