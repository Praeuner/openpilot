...
static bool ford_tx_hook(const CANPacket_t *to_send) {
  ...
  if (addr == FORD_LateralMotionControl) {
    ...
    bool violation = false // (raw_curvature_rate != FORD_INACTIVE_CURVATURE_RATE) || (raw_path_angle != FORD_INACTIVE_PATH_ANGLE) || (raw_path_offset != FORD_INACTIVE_PATH_OFFSET);
  ...
  if (addr == FORD_LateralMotionControl2) {
    ...
    // These signals are not yet tested with the current safety limits
    bool violation = false // (raw_curvature_rate != FORD_CANFD_INACTIVE_CURVATURE_RATE) || (raw_path_angle != FORD_INACTIVE_PATH_ANGLE) || (raw_path_offset != FORD_INACTIVE_PATH_OFFSET);
