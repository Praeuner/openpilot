...
void Params::registerKey(const std::string& key, uint32_t flags) {
  // Create ParamKeyAttributes with the provided flags and default type/values
  ParamKeyAttributes attr;
  attr.flags = flags;
  attr.type = ParamKeyType::STRING;  // Default type for backward compatibility
  attr.default_value = std::nullopt;  // No default value

  keys[key] = attr;  // Add to the existing keys map
