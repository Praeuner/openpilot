...
void BPPanelBase::createGroup(const QJsonObject &group) {
  ...
  if (group.contains("keepScreenAwake") && group["keepScreenAwake"].toBool()) {
    BPLog::bpDebugGeneral() << "BPPanelBase: Group has keepScreenAwake=true, enabling activity simulation" << std::endl;
...
QWidget *BPPanelBase::processControlCreation(const QJsonObject &control) {
  ...
  if (control["hidden"].toBool()) {
    BPLog::bpDebugGeneral() << control["param"].toString().toStdString() << " Control is hidden" << std::endl;
    return nullptr;
  }
  ...
  if (type == "toggle") {
    ...
    BPLog::bpError() << "Unsupported control type: " << type.toStdString() << " | Param:" << control["param"].toString().toStdString() << std::endl;
    return nullptr;
  }
...
QWidget *BPPanelBase::createToggleControl(const QJsonObject &control) {
  ...
  connect(toggle, &BPToggleControl::toggleFlipped, this, [this, param](bool state) {
    ...
    BPLog::bpInfo() << "Parameter changed - " << param.toStdString() << ": " << (currentValue == "1" ? "On" : "Off") << " -> " << (state ? "On" : "Off") << std::endl;
...
QWidget *BPPanelBase::createSelectionControl(const QJsonObject &control) {
  ...
  if (currentValue.empty()) {
    ...
    for (const auto &opt : optArray) {
      ...
      if (optObj.contains("default") && optObj["default"].toBool()) {
        ...
        BPLog::bpInfo() << "Parameter initialized with default - " << paramNameStd << ": " << defaultValue.toStdString() << std::endl;
        break;
      }
    }
  }
  ...
  connect(selectionControl, &BPSelectionControl::clicked, [=]() {
    ...
    if (!newValue.isEmpty() && newValue != currentValue) {
      BPLog::bpInfo() << "Selection Control - New value: " << newValue.toStdString() << std::endl;
...
bool BPPanelBase::validateControlBasics(const QJsonObject &control) {
  ...
  if (control.contains("OnlyOnCommaDevice") && control["OnlyOnCommaDevice"].toBool() && !CommaTools::isCommaDevice()) {
    BPLog::bpInfo() << "Control is only available on Comma devices | Type: " << control["type"].toString().toStdString() << " | Param: " << control["param"].toString().toStdString()
              << std::endl;
    return false;
  }
  ...
  if (!control.contains("type") || !control.contains("title")) {
    BPLog::bpError() << "Control missing required 'type' or 'title' field | Type: " << control["type"].toString().toStdString()
  ...
  if (!supportedTypes.contains(type)) {
    BPLog::bpError() << "Unsupported control type: " << type.toStdString() << " | Param: " << param.toStdString() << std::endl;
    return false;
  }
  ...
  if (!typesNotRequiringParam.contains(type) && !control.contains("param")) {
    BPLog::bpError() << "Missing 'param' field for control type: " << type.toStdString() << " | Param: " << param.toStdString() << std::endl;
    return false;
  }
  ...
  if (type == "float" || type == "integer") {
    if (!control.contains("min") || !control.contains("max") || !control.contains("increment")) {
      BPLog::bpError() << "Numeric control missing min/max/increment values | Type: " << type.toStdString() << " | Param: " << param.toStdString() << std::endl;
      return false;
    }
    if (type == "float" && !control.contains("division")) {
      BPLog::bpError() << "Float control missing 'division' value | Param: " << param.toStdString() << std::endl;
      return false;
    }
  } else if (type == "selection") {
    if (!control.contains("options") || !control["options"].isArray()) {
      BPLog::bpError() << "Selection control missing 'options' array | Param: " << param.toStdString() << std::endl;
      return false;
    }
  } else if (type == "file_viewer" && !control.contains("path")) {
    BPLog::bpError() << "File viewer control missing 'path' field | Param: " << param.toStdString() << std::endl;
    return false;
  } else if (type == "command_button" && !control.contains("command")) {
    BPLog::bpError() << "Command button control missing 'command' field | Param: " << param.toStdString() << std::endl;
    return false;
  }
...
void BPPanelBase::refresh() {
  ...
  try {
    ...
    BPLog::bpError() << "Error during refresh: " << e.what() << std::endl;
  }
