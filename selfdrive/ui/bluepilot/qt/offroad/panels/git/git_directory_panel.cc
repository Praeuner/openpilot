// selfdrive/ui/bluepilot/qt/offroad/panels/updater/op_updater_panel.cc
        ...
        try {
            ...
            QMetaObject::invokeMethod(this, [=]() {
                ...
                if (lastCheckTime.isValid()) {
                    statusWithTime += QString(" (%1)").arg(GitDirectoryPanel::getTimeDateString(lastCheckTime));
                }
...
GitDirectoryPanel* SubmoduleWidget::findGitManagerPanel() const {
    QWidget* parent = parentWidget();
    while (parent) {
        if (auto* panel = qobject_cast<GitDirectoryPanel*>(parent)) {
            return panel;
        }
...
GitDirectoryPanel::~GitDirectoryPanel() {
    if (autoUpdateCheckTimer) {
        autoUpdateCheckTimer->stop();
        delete autoUpdateCheckTimer;
    }
    std::cout << "GitDirectoryPanel destructor complete" << std::endl;
}

GitDirectoryPanel::GitDirectoryPanel(QWidget* parent) : QWidget(parent), branchSelector(nullptr) {
    std::cout << "GitDirectoryPanel constructor start" << std::endl;
    ...
    connect(activityTimer, &QTimer::timeout, this, &GitDirectoryPanel::simulateActivity);

    // Add automatic update check timer
    autoUpdateCheckTimer = new QTimer(this);
    autoUpdateCheckTimer->setInterval(900000);  // 15 minutes
    connect(autoUpdateCheckTimer, &QTimer::timeout, this, &GitDirectoryPanel::checkForUpdates);
    ...
    updateBranchList();
    updateButtonStates();
    std::cout << "GitDirectoryPanel constructor end" << std::endl;
}

void GitDirectoryPanel::simulateActivity() {
    ...
    if (commandInProgress) {
      ...
      std::cout << "Simulating activity in GitDirectoryPanel" << std::endl;
    }
...
void GitDirectoryPanel::stopActivitySimulation() {
    std::cout << "Stopping GitDirectoryPanel activity simulation | max duration timer stopped" << std::endl;
    activityTimer->stop();
}

void GitDirectoryPanel::resetMaxDurationTimer() {
    // Reset the max duration timer
    QTimer::singleShot(270000, this, &GitDirectoryPanel::stopActivitySimulation); // 4 minutes and 30 seconds
}

void GitDirectoryPanel::setupMainRepoSection() {
    ...
    connect(branchSelector, &BranchSelector::clicked, this, &GitDirectoryPanel::handleBranchSelection);
    ...
    // Connect signals
    connect(checkUpdatesButton, &QPushButton::clicked, this, &GitDirectoryPanel::checkForUpdates);
    connect(updateRepoButton, &QPushButton::clicked, this, &GitDirectoryPanel::handleRepoUpdate);
    connect(updateAllButton, &QPushButton::clicked, this, &GitDirectoryPanel::handleRepoUpdateAll);
    connect(repairRepoButton, &QPushButton::clicked, this, &GitDirectoryPanel::handleRepoRepair);
    connect(resetRepoButton, &QPushButton::clicked, this, &GitDirectoryPanel::handleRepoReset);
    connect(showCommitsButton, &QPushButton::clicked, this, &GitDirectoryPanel::showLastCommits);
}

void GitDirectoryPanel::setupSubmoduleSection() {
...
bool GitDirectoryPanel::isValidGitRepo() const {
    QProcess process;
    process.setWorkingDirectory(qApp->applicationDirPath() + "/../..");
    process.start("git", QStringList() << "rev-parse" << "--git-dir");
...
void GitDirectoryPanel::setupLayout() {
    QVBoxLayout* mainLayout = new QVBoxLayout(this);
    ...
    connect(refreshTimer, &QTimer::timeout, this, &GitDirectoryPanel::refreshAll);

    setLayout(mainLayout);
}

void GitDirectoryPanel::showEvent(QShowEvent *event) {
    std::cout << "Showing GitDirectoryPanel" << std::endl;
    ...
    connect(initTimer, &QTimer::timeout, this, &GitDirectoryPanel::staggeredInit);
...
void GitDirectoryPanel::hideEvent(QHideEvent *event) {
    std::cout << "Hiding GitDirectoryPanel" << std::endl;
...
void GitDirectoryPanel::startAutoUpdateChecks() {
...
void GitDirectoryPanel::stopAutoUpdateChecks() {
    if (autoUpdateCheckTimer) {
        autoUpdateCheckTimer->stop();
    }
}

void GitDirectoryPanel::refreshAll() {
    if (!shouldRefresh()) {
        return;
    }
...
void GitDirectoryPanel::updateBranchList() {
...
void GitDirectoryPanel::handleBranchSelection() {
...
void GitDirectoryPanel::switchBranch(const QString& branch) {
...
bool GitDirectoryPanel::hasUncommittedChanges() const {
    QProcess process;
    process.setWorkingDirectory(qApp->applicationDirPath() + "/../..");
    process.start("git", QStringList() << "status" << "--porcelain");
...
bool GitDirectoryPanel::hasUpdatesAvailable() const {
    QProcess process;
    process.setWorkingDirectory(qApp->applicationDirPath() + "/../..");
    ...
    if (!process.waitForFinished(5000) || process.exitCode() != 0) {
        ...
        QMetaObject::invokeMethod(const_cast<GitDirectoryPanel*>(this), [this]() {
...
void GitDirectoryPanel::checkForUpdates() {
        ...
        if (!fetchResult.success) {
            QMetaObject::invokeMethod(this, [=]() {
                ...
                if (isVisible() && !errorDialogShowing) {
                    ...
                    connect(errorDialog, &QDialog::finished, [this]() {
...
void GitDirectoryPanel::staggeredInit() {
    if (initStage >= 5) {  // Changed from 4 to 5 to add new stage
        ...
        if (currentSubmoduleIndex < submoduleWidgets.size()) {
            std::cout << "GitDirectoryPanel: Stage 5 - Refreshing submodule " << currentSubmoduleIndex << std::endl;
            QTimer::singleShot(100, this, [this]() {
                auto* widget = submoduleWidgets[currentSubmoduleIndex];
                if (widget) {
                    widget->refresh();
                    std::cout << "GitDirectoryPanel: Refreshed submodule " << currentSubmoduleIndex << std::endl;
                } else {
                    std::cerr << "GitDirectoryPanel: Widget is null for submodule " << currentSubmoduleIndex << std::endl;
                }
            ...
            std::cout << "GitDirectoryPanel: Stage 5 - All submodules processed, stopping init timer" << std::endl;
            if(initTimer) {
                initTimer->stop();
                delete initTimer;
                initTimer = nullptr;
    ...
    switch(initStage++) {
    case 0:
        std::cout << "GitDirectoryPanel: Stage 0 - Just showing UI" << std::endl;
        break;
    case 1:
        std::cout << "GitDirectoryPanel: Stage 1 - Updating branch list" << std::endl;
        QTimer::singleShot(100, this, [this]() {
            updateBranchList();
        });
        break;
    case 2:
        std::cout << "GitDirectoryPanel: Stage 2 - Refreshing main repo status" << std::endl;
        ...
        break;
    case 3:
        std::cout << "GitDirectoryPanel: Stage 3 - Checking local changes and updates" << std::endl;
        ...
        break;
    case 4:
        std::cout << "GitDirectoryPanel: Stage 4 - Checking for updates" << std::endl;
...
QString GitDirectoryPanel::getTimeAgoString(const QDateTime& time) {
...
QString GitDirectoryPanel::getTimeDateString(const QDateTime& time) {
    if (!time.isValid()) return "";
    return time.toString("MM/dd h:mm AP");
}

void GitDirectoryPanel::updateCheckUpdatesButtonText() {
...
void GitDirectoryPanel::showCommandOutputDialog(const QString& title, const QString& command,
...
void GitDirectoryPanel::showCommitHistory(QWidget* parent, const QString& title, const QString& workingDir) {
...
void GitDirectoryPanel::handleRepoUpdate() {
...
void GitDirectoryPanel::handleRepoUpdateAll() {
    // If there are uncommitted local changes, confirm before proceeding
    if (hasUncommittedChanges()) {
      if (!ConfirmationDialog::confirm(
            tr("You have local changes that will be overwritten by this update. Continue?"),
...
void GitDirectoryPanel::handleRepoRepair() {
    if (!ConfirmationDialog::confirm(
...
void GitDirectoryPanel::handleRepoReset() {
    if (ConfirmationDialog::confirm(
...
void GitDirectoryPanel::showLastCommits() {
...
void GitDirectoryPanel::showErrorState(const QString& message) {
    QVBoxLayout* mainLayout = new QVBoxLayout(this);
    mainLayout->setSpacing(20);
    mainLayout->setContentsMargins(10, 10, 10, 10);
...
bool GitDirectoryPanel::isInternetAvailable() const {
    // return false;
...
bool GitDirectoryPanel::isSSHValid() const {
    // If we have a recent check, reuse that result
...
bool GitDirectoryPanel::checkRootDiskSpace() {
...
bool GitDirectoryPanel::repairRootDiskSpace() {
...
bool GitDirectoryPanel::checkAndRestoreSSH() {
...
bool GitDirectoryPanel::restoreSSHFromUtility() {
...
void GitDirectoryPanel::updateStatusLabel(UpdaterStatus status) const {
...
void GitDirectoryPanel::updateButtonStates() {
    bool internetAvailable = isInternetAvailable();
...
void GitDirectoryPanel::setupParamMonitoring() {
    QTimer* paramCheckTimer = new QTimer(this);
