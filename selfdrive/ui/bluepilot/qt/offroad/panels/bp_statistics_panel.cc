...
BPStatisticsPanel::BPStatisticsPanel(QWidget *parent) : QWidget(parent) {
  setMouseTracking(true);
  setMinimumWidth(1000);
  setMaximumWidth(1920);
  setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Preferred);

  // Style the panel similar to BPPanel
...
void BPStatisticsPanel::paintEvent(QPaintEvent *) {
  QPainter p(this);
  p.fillRect(rect(), Qt::transparent);
}

void BPStatisticsPanel::setupUI() {
  // Create main layout
  ...
  container->setStyleSheet("QWidget#statsContainer { background: transparent; }");
  container->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Minimum); // Allow container to grow

  QVBoxLayout *containerLayout = new QVBoxLayout(container);
  containerLayout->setSpacing(50);
  containerLayout->setContentsMargins(25, 0, 25, 25);

  // Create the stat cards with titles and descriptions matching the JSON
  systemCard = new SystemStatCard(container);
  connectivityCard = new ConnectivityStatCard(container);
  storageCard = new StorageStatCard(container);

  containerLayout->addWidget(systemCard);
  containerLayout->addWidget(connectivityCard);
  containerLayout->addWidget(storageCard);
  containerLayout->addStretch();

  // Create scroll view and add container
  BPScrollView *scrollView = new BPScrollView(container, this);
  scrollView->setFrameShape(QFrame::NoFrame);
  scrollView->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding); // Allow scroll view to expand
  mainLayout->addWidget(scrollView);
}

void BPStatisticsPanel::showEvent(QShowEvent *event) {
  isVisible = true;
  if (systemCard)
    systemCard->startUpdates();
  if (connectivityCard)
    connectivityCard->startUpdates();
  if (storageCard)
    storageCard->startUpdates();

  // Request activity simulation if needed
  if (keepScreenAwake) {
...
void BPStatisticsPanel::hideEvent(QHideEvent *event) {
  isVisible = false;
  if (systemCard)
    systemCard->stopUpdates();
  if (connectivityCard)
    connectivityCard->stopUpdates();
  if (storageCard)
    storageCard->stopUpdates();
...
BPStatisticsPanel::~BPStatisticsPanel() {
...
//---------------------------------------
// BPBarGauge Implementation
//---------------------------------------
BPBarGauge::BPBarGauge(QWidget *parent) : QWidget(parent) {
  setMinimumHeight(45);
  gradientStops = DefaultGradientThreeColor();
}

void BPBarGauge::setValue(float value) {
  currentValue = std::clamp(value, 0.0f, 100.0f);
  update();
}

void BPBarGauge::setGradientStops(const QVector<GradientStop> &stops) {
  gradientStops = stops;
  update();
}

QString BPBarGauge::generateGradientStyle() {
  // Find appropriate gradient based on current value
  QString gradient;
  for (int i = 0; i < gradientStops.size(); i++) {
    if (currentValue <= gradientStops[i].value) {
      QVector<QString> gradientColors;
      if (i > 0) {
        gradientColors = gradientStops[i - 1].colors;
      } else {
        gradientColors = gradientStops[0].colors;
      }
      ...
      for (int j = 0; j < gradientColors.size(); j++) {
        ...
        gradient += QString(", stop:%1 %2").arg(stopPosition, 0, 'f', 2).arg(gradientColors[j]);
      }
      gradient += ")";
      break;
    }
  ...
  // If no match found, use last gradient
  if (gradient.isEmpty() && !gradientStops.isEmpty()) {
    const auto &lastColors = gradientStops.last().colors;
    gradient = "qlineargradient(x1:0, y1:0, x2:1, y2:0";
    float step = 1.0f / (lastColors.size() - 1);
    for (int j = 0; j < lastColors.size(); j++) {
      float stopPosition = j * step;
      gradient += QString(", stop:%1 %2").arg(stopPosition, 0, 'f', 2).arg(lastColors[j]);
    }
    gradient += ")";
  }
...
void BPBarGauge::paintEvent(QPaintEvent *) {
  ...
  if (currentValue > 0) {
    ...
    progressRect.setWidth(rect().width() * (currentValue / 100.0));

    QLinearGradient gradient(QPointF(0, 0), QPointF(1, 0));
    gradient.setCoordinateMode(QGradient::ObjectBoundingMode);

    // Apply gradient stops based on value
    for (int i = 0; i < gradientStops.size(); i++) {
      if (currentValue <= gradientStops[i].value) {
        const auto &colors = gradientStops[i].colors;
        float step = 1.0f / (colors.size() - 1);
        for (int j = 0; j < colors.size(); j++) {
          gradient.setColorAt(j * step, QColor(colors[j]));
        }
        break;
      }
    }
  ...
  QString text = QString("%1%").arg(QString::number(currentValue, 'f', 1));
  painter.drawText(rect(), Qt::AlignCenter, text);
}

//---------------------------------------
// BPProgressBar Implementation
//---------------------------------------
BPProgressBar::BPProgressBar(QWidget *parent) : QWidget(parent) {
  setMinimumHeight(45);

  gradient.setStart(QPointF(0, 0));
  gradient.setFinalStop(QPointF(1, 0));
  gradient.setCoordinateMode(QGradient::ObjectBoundingMode);

  gradient.setColorAt(0.0, QColor("#4CAF50")); // Material Green
  gradient.setColorAt(0.5, QColor("#FFC107")); // Material Amber
  gradient.setColorAt(1.0, QColor("#F44336")); // Material Red
}

void BPProgressBar::setValue(double value) {
  currentValue = std::clamp(value, 0.0, 100.0);
  update();
}

void BPProgressBar::setGradient(const QLinearGradient &newGradient) {
  gradient = newGradient;
  update();
}

void BPProgressBar::paintEvent(QPaintEvent *) {
  QPainter painter(this);
  painter.setRenderHint(QPainter::Antialiasing);
  ...
  font.setBold(true);
  painter.setFont(font);
  painter.drawText(rect(), Qt::AlignCenter, QString("%1%").arg(QString::number(currentValue, 'f', 1)));
}

//---------------------------------------
// StatCardBase Implementation
//---------------------------------------
StatCardBase::StatCardBase(const QString &title, const QString &desc, const QString &icon, QWidget *parent) : QWidget(parent) {
...
QString StatCardBase::formatPercentage(float percentage) { return QString("%1%").arg(percentage, 0, 'f', 1); }

QLabel *StatCardBase::createHeaderLabel(const QString &text, int fontSize, const QString &color) {
...
QLabel *StatCardBase::createDescriptionLabel(const QString &text, int fontSize, const QString &color) {
  QLabel *label = new QLabel(text, container);
...
QLabel *StatCardBase::createDataLabel(const QString &text, int fontSize, const QString &color) {
  QLabel *label = new QLabel(text, container);
...
//---------------------------------------
SystemStatCard::SystemStatCard(QWidget *parent) : StatCardBase(tr("System Statistics"), "", "", parent) {

  QTimer::singleShot(0, this, [this]() {
    updateCardStyle();

    // Create main horizontal layout for CPU and Memory cards
    ...
    QLabel *cpuHeader = createHeaderLabel(tr("CPU Usage"), 36, "#FFFFFF");
    cpuLayout->addWidget(cpuHeader);

    cpuGauge = new BPBarGauge(this);
    cpuGauge->setGradientStops(DetailedGradientFiveColor());
    ...
    QLabel *memHeader = createHeaderLabel(tr("Memory Usage"), 36, "#FFFFFF");
    memLayout->addWidget(memHeader);

    memoryGauge = new BPBarGauge(this);
    memoryGauge->setGradientStops(DetailedGradientFiveColor());
    memLayout->addWidget(memoryGauge);

    // Memory details
    memDetailsLabel = createDescriptionLabel("", 32, "#dddddd");
    memDetailsLabel->setWordWrap(true);
    memLayout->addWidget(memDetailsLabel);
    memLayout->addStretch();

    // Add cards to main layout with appropriate sizing
...
void SystemStatCard::refresh() {
  ...
  runAsync<SystemMetrics>([this]() { return gatherMetrics(); }, [this](const SystemMetrics &metrics) { updateDisplay(metrics); });
}
...
SystemMetrics SystemStatCard::gatherMetrics() {
  ...
  metrics.memTotal = values["Pages free"] + values["Pages active"] + values["Pages inactive"] + values["Pages speculative"] + values["Pages wired down"];
  ...
  // Linux top processes
  {
    ...
    std::sort(sortedProcesses.begin(), sortedProcesses.end(), [](const auto &a, const auto &b) { return a.first > b.first; });

    // Take top processes
    metrics.topProcesses.clear();
    for (int i = 0; i < qMin(10, sortedProcesses.size()); ++i) {
      ProcessInfo p;
...
void SystemStatCard::updateDisplay(const SystemMetrics &metrics) {
  ...
  float memPercent = (metrics.memTotal > 0) ? (float(metrics.memUsed) / metrics.memTotal) * 100.0f : 0.0f;
...
//---------------------------------------
// ConnectivityStatCard Implementation
//---------------------------------------
ConnectivityStatCard::ConnectivityStatCard(QWidget *parent) : StatCardBase(tr("Connectivity Statistics"), "", "", parent) {
  ...
  QTimer::singleShot(0, this, [this]() {
    ...
    wifiHeaderLabel = createHeaderLabel(tr("Wi-Fi Connection"), 36, "#FFFFFF");
    ...
    // Create labels with BP styling
    wifiSSIDLabel = createDescriptionLabel("", 32, "#dddddd");
    wifiSignalStrengthLabel = createDescriptionLabel("", 32, "#dddddd");
    wifiIPAddressLabel = createDescriptionLabel("", 32, "#dddddd");

    // Signal strength bar
    wifiSignalBar = new BPProgressBar(this);
    QLinearGradient wifiGradient;
    wifiGradient.setColorAt(0.0, QColor("#9ece6a")); // Strong signal
    wifiGradient.setColorAt(1.0, QColor("#f7768e")); // Weak signal
    wifiSignalBar->setGradient(wifiGradient);

    QHBoxLayout *wifiSpeedLayout = new QHBoxLayout();
    wifiRXSpeedLabel = createDataLabel("", 32, "#dddddd");
    wifiTXSpeedLabel = createDataLabel("", 32, "#dddddd");
    ...
    wifiFrameLayout->addWidget(wifiSignalBar);
    ...
    cellularHeaderLabel = createHeaderLabel(tr("Cellular Connection"), 36, "#FFFFFF");
    ...
    // Create labels with BP styling
    signalStrengthLabel = createDescriptionLabel("", 32, "#dddddd");
    carrierLabel = createDescriptionLabel("", 32, "#dddddd");
    networkTypeLabel = createDescriptionLabel("", 32, "#dddddd");
    registrationStateLabel = createDescriptionLabel("", 32, "#dddddd");

    // Cellular signal strength bar
    cellSignalBar = new BPProgressBar(this);
    QLinearGradient cellGradient;
    cellGradient.setColorAt(0.0, QColor("#9ece6a")); // Strong signal
    cellGradient.setColorAt(1.0, QColor("#f7768e")); // Weak signal
    cellSignalBar->setGradient(cellGradient);

    cellularFrameLayout->addWidget(signalStrengthLabel);
    cellularFrameLayout->addWidget(cellSignalBar);
...
void ConnectivityStatCard::refresh() {
  ...
  runAsync<ConnectivityInfo>([this]() { return gatherConnectivityInfo(); }, [this](const ConnectivityInfo &info) { updateDisplay(info); });
}
...
ConnectivityStatCard::ConnectivityInfo ConnectivityStatCard::gatherConnectivityInfo() {
  ...
  info.cellular.signalQuality = info.cellular.signalStrength > 80 ? "Excellent" : info.cellular.signalStrength > 60 ? "Good" : info.cellular.signalStrength > 40 ? "Fair" : "Poor";
  ...
  if (match.hasMatch()) {
    ...
    if (ok && rawSignal >= 0 && rawSignal <= 100) {
      ...
      info.cellular.signalQuality = info.cellular.signalStrength > 80   ? "Excellent"
                                    ...
                                    : info.cellular.signalStrength > 40 ? "Fair"
                                                                        : "Poor";
    }
  }
  ...
  info.wifi.signalQuality = info.wifi.signalStrength > 80 ? "Excellent" : info.wifi.signalStrength > 60 ? "Good" : info.wifi.signalStrength > 40 ? "Fair" : "Poor";
  ...
  for (const QString &line : lines) {
    if (line.startsWith('*')) { // Connected network
      ...
      if (parts.size() >= 3) {
        ...
        info.wifi.signalQuality = signal > 80 ? "Excellent" : signal > 60 ? "Good" : signal > 40 ? "Fair" : "Poor";
        break;
      }
    }
  }
  ...
  interfaceIpProcess.start("ip", QStringList() << "addr" << "show" << "wlan0");
  ...
  // RX and TX Speeds
  static qint64 lastRxBytes = 0;
  static qint64 lastTxBytes = 0;
  ...
  if (netDev.open(QIODevice::ReadOnly)) {
    ...
    while (!netDev.atEnd()) {
      ...
      if (line.contains("wlan0")) {
        ...
        if (fields.size() >= 10) {
          qint64 rxBytes = fields[1].toLongLong();
          qint64 txBytes = fields[9].toLongLong();

          QDateTime now = QDateTime::currentDateTime();
          double seconds = lastUpdate.msecsTo(now) / 1000.0;

          if (seconds > 0) {
            info.wifi.rxSpeed = (rxBytes - lastRxBytes) / (seconds * 1024 * 1024); // MB/s
            info.wifi.txSpeed = (txBytes - lastTxBytes) / (seconds * 1024 * 1024); // MB/s
          }
...
void ConnectivityStatCard::updateDisplay(const ConnectivityInfo &info) {
  ...
  if (info.cellular.signalStrength != -1) {
    ...
    cellSignalBar->setValue(info.cellular.signalStrength);
    signalStrengthLabel->setText(
        QString("Signal Strength: <span style='color: %1;'>%2% (%3)</span>").arg(signalColor).arg(info.cellular.signalStrength).arg(info.cellular.signalQuality));
  } else {
  ...
  registrationStateLabel->setText(QString("Registration State: <span style='color: %1;'>%2</span>").arg(regColor).arg(info.cellular.registrationState));
  ...
  if (info.wifi.signalStrength != -1) {
    ...
    wifiSignalBar->setValue(info.wifi.signalStrength);
    wifiSignalStrengthLabel->setText(
        QString("Signal Strength: <span style='color: %1;'>%2% (%3)</span>").arg(wifiSignalColor).arg(info.wifi.signalStrength).arg(info.wifi.signalQuality));
  } else {
  ...
  // RX and TX Speeds
  wifiRXSpeedLabel->setText(QString("↓ <span style='color: #9ece6a;'>%1 MB/s</span>").arg(QString::number(info.wifi.rxSpeed, 'f', 1)));
  wifiTXSpeedLabel->setText(QString("↑ <span style='color: #f7768e;'>%1 MB/s</span>").arg(QString::number(info.wifi.txSpeed, 'f', 1)));
}

//---------------------------------------
// StorageStatCard Implementation
//---------------------------------------
StorageStatCard::StorageStatCard(QWidget *parent) : StatCardBase(tr("Storage Statistics"), "", "", parent) {

  QTimer::singleShot(0, this, [this]() {
    this->updateCardStyle();

    // Create scrollable area for storage devices
    ...
    connect(storageProcess, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this, &StorageStatCard::handleStorageDataReady);
  ...
  runAsync<QList<StorageInfo>>(
      [this]() {
        ...
        for (const QStorageInfo &storage : QStorageInfo::mountedVolumes()) {
          ...
          if (device.startsWith("/dev/") && !storage.isReadOnly() && !storage.fileSystemType().contains("tmpfs")) {
      ...
      [this](const QList<StorageInfo> &devices) { updateStorageDisplay(devices); });
#endif
}
  ...
  for (int i = 1; i < lines.size(); i++) {
    ...
    if (fields.size() >= 9) {
      ...
      if (mountPoint == "/" || mountPoint.startsWith("/Volumes/")) {
        ...
        if (storage.isValid()) {
          QString name = mountPoint == "/" ? "System" : QDir(mountPoint).dirName();
...
void StorageStatCard::updateStorageDisplay(const QList<StorageInfo> &devices) {
  ...
  for (const StorageInfo &device : devices) {
    ...
    if (!deviceFrames.contains(deviceKey)) {
      ...
      mountLabel = createHeaderLabel("", 36, "#FFFFFF");
      cardLayout->addWidget(mountLabel);

      gauge = new BPBarGauge(deviceFrame);
      cardLayout->addWidget(gauge);

      detailsLabel = createDescriptionLabel("", 32, "#dddddd");
...
//---------------------------------------
// StatCardFactory Implementation
//---------------------------------------
StatCardBase *StatCardFactory::createStatsCard(const QString &type, QWidget *parent) {
  if (type == "stats_card_system") {
    return new SystemStatCard(parent);
  } else if (type == "stats_card_connectivity") {
    return new ConnectivityStatCard(parent);
  } else if (type == "stats_card_storage") {
    return new StorageStatCard(parent);
  }
  return nullptr;
}
