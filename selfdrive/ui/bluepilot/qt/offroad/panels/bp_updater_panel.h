// selfdrive/ui/bluepilot/qt/offroad/panels/bp_updater_panel.h

#pragma once

// Forward declaration first
class BPUpdaterPanel;
...
public:
  ...
  void getBranchesAsync(bool includeRemote, std::function<void(QStringList)> callback);

signals:
  void clicked();
  void branchLoadingStarted();
  void branchLoadingFinished();
...
private:
  ...
  GitCommandResult executeGitCommand(const QString &command, const QString &workingDir, int timeoutMs = 30000) const;
};
...
private:
  ...
  BPUpdaterPanel *findGitManagerPanel() const;
};

class BPUpdaterPanel : public QWidget {
  Q_OBJECT
public:
  explicit BPUpdaterPanel(QWidget *parent = nullptr);
  ~BPUpdaterPanel();
...
private:
  ...
  bool shouldRefresh() { return lastRefreshTime.isNull() || lastRefreshTime.secsTo(QDateTime::currentDateTime()) > 30; }
  ...
  static CommandResult executeGitCommand(const QString &command, const QString &workingDir, int timeoutMs = 30000) {
  ...
  enum class UpdaterStatus { OK, NO_INTERNET, SSH_MISSING, SSH_AUTH_FAILED, NO_REMOTE_BRANCH, ONROAD };

  // Define message mapping using tuples
  const std::vector<std::tuple<UpdaterStatus, QString>> STATUS_MESSAGES = {{UpdaterStatus::OK, ""},
...
class BPUpdaterSelectionDialog : public QDialog {
  Q_OBJECT
public:
  static QString getSelection(const QString &title, const QStringList &options, const QString &current, QWidget *parent = nullptr);

protected:
  explicit BPUpdaterSelectionDialog(QWidget *parent = nullptr);
  void setupDialog(const QString &title, const QStringList &options, const QString &current);
...
class BPUpdateConfirmDialog : public BPDialogBase {
  Q_OBJECT
public:
  explicit BPUpdateConfirmDialog(const QString &title, const QString &prompt, const QString &confirm_text, const QString &cancel_text, QWidget *parent = nullptr);
  static bool confirm(const QString &title_text, const QString &prompt_text, const QString &confirm_text = tr("Yes"), const QString &cancel_text = tr("No"),
                      QWidget *parent = nullptr);
  static void alert(const QString &message, QWidget *parent = nullptr);
};

class BPScrollArea : public QScrollArea {
  Q_OBJECT
public:
  explicit BPScrollArea(QWidget *parent = nullptr) : QScrollArea(parent) {
    ...
    props.setScrollMetric(QScrollerProperties::DragStartDistance, QVariant::fromValue(0.001));
    props.setScrollMetric(QScrollerProperties::MinimumVelocity, QVariant::fromValue(0.0));
    props.setScrollMetric(QScrollerProperties::MaximumVelocity, QVariant::fromValue(0.5));
    props.setScrollMetric(QScrollerProperties::AcceleratingFlickMaximumTime, QVariant::fromValue(0.4));
