...
struct DecoderManager {
  VideoDecoder *acquire(CameraType type, AVCodecParameters *codecpar, bool hw_decoder) {
    ...
    // For QCOM decoders, don't reuse to avoid V4L2 state issues
    bool is_qcom = false;
    #ifndef __APPLE__
    is_qcom = (Hardware::TICI() && hw_decoder);
    #endif
    ...
    if (!is_qcom && decoder) {
      ...
      // For QCOM (or fallback), return raw pointer but don't store in map
      current_qcom_decoder_ = std::move(decoder);
      return current_qcom_decoder_.get();
    }
...
private:
  ...
  std::unique_ptr<VideoDecoder> current_qcom_decoder_;
};
...
bool FFmpegVideoDecoder::copyBuffer(AVFrame *f, VisionBuf *buf) {
  ...
  if (f->format == AV_PIX_FMT_YUV420P) {
    ...
    // Convert U and V planes to interleaved UV (NV12) with proper alignment
    for (int i = 0; i < height / 2; i++) {
      uint8_t *uv_row = uv_plane + i * buf->stride;
      uint8_t *u_row = f->data[1] + i * f->linesize[1];
      uint8_t *v_row = f->data[2] + i * f->linesize[2];

      for (int j = 0; j < width / 2; j++) {
        uv_row[j * 2] = u_row[j];     // U
        uv_row[j * 2 + 1] = v_row[j]; // V
      }
    }
    ...
    for (int i = 0; i < height / 2; i++) {
      memcpy(uv_plane + i * buf->stride, f->data[1] + i * f->linesize[1], width);
    }
