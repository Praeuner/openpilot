...
void BPRoutesPanel::loadRoutes() {
  ...
  BPLog::bpWarn() << "[bp.routes.panel] loadRoutes | ===== LOAD ROUTES STARTED =====" << std::endl;

  if (isLoading) {
    BPLog::bpWarn() << "[bp.routes.panel] loadRoutes | Already loading - skipping" << std::endl;
    return;
  }

  // Safety check: Don't load routes while onroad
  if (uiState()->scene.started) {
    BPLog::bpWarn() << "[bp.routes.panel] loadRoutes | Aborting route loading - device is onroad" << std::endl;
    showOnroadMessage();
    return;
  }
  ...
  if (cacheLoaded && routeCache.isValid() && !shouldRefreshRoutes() && !routeCache.routeInfoCache.isEmpty()) {
    ...
    QMetaObject::invokeMethod(this, [this, cachedRoutes, startTime]() {
      routes = cachedRoutes;
      isLoading = false;
      hideLoadingOverlay();
      updateStats();
      ...
      BPLog::bpWarn() << "[bp.routes.panel] loadRoutes | ===== LOAD ROUTES COMPLETED (from cache) in " << duration << "ms =====" << std::endl;
  ...
  QtConcurrent::run([this, startTime]() {
    TRACK_CONCURRENT_TASK("BPRoutesPanel::loadRoutes");
    ...
    QMetaObject::invokeMethod(this, [this, newRoutes, startTime]() {
      routes = newRoutes;
      ...
      auto endTime = std::chrono::steady_clock::now();
      auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime).count();
      BPLog::bpWarn() << "[bp.routes.panel] loadRoutes | ===== LOAD ROUTES COMPLETED (fresh scan) in " << duration << "ms =====" << std::endl;
...
QString BPRoutesPanel::generateThumbnailAuto(const QString &routeBase) {
  TRACK_CONCURRENT_TASK(QString("BPRoutesPanel::generateThumbnail(%1)").arg(routeBase));
  // Check if thumbnail already exists
  QString thumbnailPath = getThumbnailPath(routeBase);
  if (QFile::exists(thumbnailPath)) {
    return thumbnailPath;
...
void BPRoutesPanel::showOnroadMessage() {
  ...
  if (!onroadMessageWidget) {
    ...
    // Header section (reuse existing header but with disabled buttons)
    QWidget *headerWidget = new QWidget;
    headerWidget->setFixedHeight(120);
    headerWidget->setStyleSheet("background: transparent;");
    ...
    QLabel *onroadTitleLabel = new QLabel("Driving Routes");
    onroadTitleLabel->setStyleSheet("font-size: 48px; font-weight: 600; color: white;");

    QLabel *onroadStatsLabel = new QLabel("1 routes • 2.4 GB");
    onroadStatsLabel->setStyleSheet("font-size: 32px; color: #cccccc;");

    titleStatsLayout->addWidget(onroadTitleLabel);
    titleStatsLayout->addWidget(onroadStatsLabel);

    headerLayout->addLayout(titleStatsLayout, 1);

    // Disabled action buttons
    QPushButton *disabledRefreshButton = new QPushButton("Refresh");
    disabledRefreshButton->setFixedSize(180, 80);
    disabledRefreshButton->setEnabled(false);
    disabledRefreshButton->setStyleSheet(R"(
      QPushButton {
        background-color: #666666;
        color: #999999;
        font-size: 28px;
    ...
    QPushButton *disabledClearCacheButton = new QPushButton("Clear Cache");
    disabledClearCacheButton->setFixedSize(180, 80);
    disabledClearCacheButton->setEnabled(false);
    disabledClearCacheButton->setStyleSheet(disabledRefreshButton->styleSheet());

    headerLayout->addWidget(disabledRefreshButton);
    headerLayout->addWidget(disabledClearCacheButton);

    layout->addWidget(headerWidget);

    // Disable message section
    QWidget *messageSection = new QWidget;
    QVBoxLayout *messageLayout = new QVBoxLayout(messageSection);
    messageLayout->setAlignment(Qt::AlignCenter);
    messageLayout->setSpacing(40);
    ...
    messageLayout->addWidget(safetyLabel);

    layout->addWidget(messageSection, 1); // Give it stretch factor to use remaining space
...
void BPRoutesPanel::onOffroadTransition() {
  BPLog::bpInfo() << "[bp.routes.panel] onOffroadTransition | started=" << uiState()->scene.started << std::endl;

  if (uiState()->scene.started) {
    // Device went onroad - show safety message and clear routes
    BPLog::bpInfo() << "[bp.routes.panel] onOffroadTransition | Device went onroad - disabling routes panel" << std::endl;
    showOnroadMessage();
    ...
    if (QWidget *topLevel = window()) {
      ...
      for (QDialog* dialog : dialogs) {
        if (dialog->isVisible()) {
          BPLog::bpInfo() << "[bp.routes.panel] onOffroadTransition | Closing video dialog due to onroad transition" << std::endl;
          dialog->close();
        }
      }
    }
  } else {
    // Device went offroad - clear message and DEFER route loading to avoid blocking UI thread
    BPLog::bpInfo() << "[bp.routes.panel] onOffroadTransition | Device went offroad - enabling routes panel" << std::endl;
    clearOnroadMessage();

    // CRITICAL FIX: Don't load routes synchronously during transition - defer to avoid watchdog timeout
    // Routes will be loaded when the panel becomes visible (showEvent) or after a short delay
    if (routes.isEmpty() && isVisible()) {
      BPLog::bpWarn() << "[bp.routes.panel] onOffroadTransition | DEFERRING route load to prevent UI blocking (was visible)" << std::endl;
      // Defer loading by 500ms to let the UI transition complete
      QTimer::singleShot(500, this, [this]() {
        if (!uiState()->scene.started && isVisible()) {
          BPLog::bpInfo() << "[bp.routes.panel] onOffroadTransition | Deferred route load executing now" << std::endl;
          loadRoutes();
        }
      });
    } else if (routes.isEmpty()) {
      BPLog::bpInfo() << "[bp.routes.panel] onOffroadTransition | Panel not visible - deferring route loading to showEvent" << std::endl;
    }
  }
}
