...
void OnroadAlertsBP::paintEvent(QPaintEvent *event) {
  ...
  static std::map<cereal::SelfdriveState::AlertSize, const int> alert_heights = {
    {cereal::SelfdriveState::AlertSize::SMALL, 140},  // Reduced height
    {cereal::SelfdriveState::AlertSize::MID, 280},    // Reduced height
  ...
  int margin = isFullscreen ? 0 : 40;
  QRect r = QRect(margin, height() - h - margin, width() - margin*2, h);
  ...
  if (alert.size == cereal::SelfdriveState::AlertSize::SMALL) {
    ...
    QRect text1Rect = textContainer.adjusted(20, 50, -20, 0);
    ...
    // Draw secondary text (no strong shadow needed)
    ...
    p.drawText(textContainer.adjusted(20, 140, -20, -20), 
               Qt::AlignHCenter | Qt::AlignTop | Qt::TextWordWrap, displayText);
    ...
    p.setFont(InterFont(longText ? 110 : 144, QFont::Bold));
    
    QRect text1Rect = QRect(0, r.y() + (longText ? 200 : 220), width(), 400);
    ...
    // Secondary text
    p.setFont(InterFont(72, QFont::DemiBold));
    p.setPen(QColor(220, 220, 220));
    p.drawText(QRect(0, r.height() - (longText ? 300 : 360), width(), 250), 
