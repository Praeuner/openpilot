...
void LateralGraphWidget::paintEvent(QPaintEvent *event) {
  ...
  // Automotive-style border
  QLinearGradient borderGradient(rect().topLeft(), rect().bottomLeft());
  borderGradient.setColorAt(0, QColor(100, 149, 237, 150)); // Cornflower blue
  borderGradient.setColorAt(1, QColor(70, 130, 180, 150));  // Steel blue
  ...
  // Draw graph background with automotive inset effect
  ...
  // Draw grid lines with automotive styling
  ...
  p.setPen(QColor(236, 240, 241, 230));  // Slightly off-white
  int scale_x = 20;
