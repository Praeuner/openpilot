...
void HudRendererSP::draw(QPainter &p, const QRect &surface_rect) {
  ...
  // Standstill Timer (disabled for BluePilot - has its own standalone overlay)
#ifndef BLUEPILOT
...
void HudRendererSP::drawWithModel(QPainter &p, const QRect &surface_rect, const ModelType &model) {
  // First draw the standard HUD elements
  draw(p, surface_rect);

  // Then integrate BluePilot renderer with the actual model
  const UIState *s = uiState();
  if (!reversing) {
    BluepilotRenderer::renderAll(p, surface_rect, *s, model);
  }
}

// Explicit template instantiation for BluePilot
#ifdef BLUEPILOT
template void HudRendererSP::drawWithModel<ModelRendererBP>(QPainter &p, const QRect &surface_rect, const ModelRendererBP &model);
#endif
...
void HudRendererSP::drawE2eAlert(QPainter &p, const QRect &surface_rect, const QString &alert_alt_text) {
  ...
  // Position alert on right side of screen
  int card_width = 480;
  int card_height = 160;
  int x = surface_rect.right() - card_width - (devUiInfo > 0 ? 200 : 120) - (UI_BORDER_SIZE * 3);
  int y = surface_rect.center().y() - card_height / 2 + 20;
  QRect cardRect(x, y, card_width, card_height);

  // Pulsing glow effect
  bool pulse = (e2eAlertFrame % 40 < 20);
  int glow_alpha = pulse ? 90 : 50;
  QRect glowRect = cardRect.adjusted(-10, -10, 10, 10);
  p.setPen(Qt::NoPen);
  p.setBrush(QColor(0, 255, 0, glow_alpha));
  p.drawRoundedRect(glowRect, 28, 28);

  // Main card with gradient background
  QLinearGradient gradient(cardRect.topLeft(), cardRect.bottomRight());
  gradient.setColorAt(0, QColor(20, 20, 20, 240));
  gradient.setColorAt(1, QColor(10, 10, 10, 240));
  p.setBrush(gradient);
  p.setPen(QPen(QColor(0, 255, 0, 150), 4));
  p.drawRoundedRect(cardRect, 24, 24);

  // Icon on left side
  int icon_size = 90;
  QRect iconRect(x + 30, y + (card_height - icon_size) / 2, icon_size, icon_size);
  if (!alert_img.isNull()) {
    p.drawPixmap(iconRect, alert_img.scaled(icon_size, icon_size, Qt::KeepAspectRatio, Qt::SmoothTransformation));
  }

  // Text on right side
  QRect textRect(x + 135, y + 20, card_width - 160, card_height - 40);
  p.setFont(InterFont(44, QFont::Bold));
  p.setPen(QColor(0, 255, 0, 255));
  p.drawText(textRect, Qt::AlignLeft | Qt::AlignVCenter, alert_text);
}
...
void HudRendererSP::drawBlinker(QPainter &p, const QRect &surface_rect) {
  ...
  const int x_gap = 240;  // Match BluePilot positioning
  const int y_offset = 150;  // Match BluePilot positioning
